...
function draw()　{
  ...
  if (!drawingCards){ //If the player is not drawing cards
    ...
    if (dash) {
      playerX = min(max(playerX + dashDir.x*Speed*scaling*delta*speedMultiplier*((abilityTimer*3 + 60)/60), width*-1.5+playerSize/2), width*1.5-playerSize/2);
      playerY = min(max(playerY + dashDir.y*Speed*scaling*delta*speedMultiplier*((abilityTimer*3 + 60)/60), height*-1.5+playerSize/2), height*1.5-playerSize/2);
    }
    ...
    if (roundsQued > 0 && roundTimer <= 0) {
      ...
      roundsQued--;
      roundTimer = min(5, shootCooldown/roundsQued);
    }
    fill(25, 50, 75);
    ...
    healthScale += delta/600*pow(diffStatScale, 2);
    ...
    if (ability != 0) {
      ...
      if (ability == 2 && !spike) {
        ...
        rect(50*scaling + (180-abilityCooldown)/180*200*scaling, height-100*scaling, (180-abilityCooldown)/180*400*scaling, 25*scaling);
      }
        ...
        rect(50*scaling + abilityTimer/60*200*scaling, height-100*scaling, abilityTimer/60*400*scaling, 25*scaling);
      }
      else if (spike) {
        ...
        switch(ceil(random(4))) //Chooses random number 1 to 4 and spawns enemies on the side of the screen corresponding to that number
        {
          case 1: //Spawn enemy on top edge of screen
              if (playerY - 80*scaling - height/2 > -1.5*height) {
                  NMEs.push(new NME(-1*(translateX-width/2)+spawnPosX*width, min(playerY-height/2-40*scaling, height/2-40*scaling), 5*((healthScale-1)/1.5+1)*diffStatScale, 1));
                }
              else { //Spawns enemy on bottom edge of screen if top edge is outside of border
                  NMEs.push(new NME(-1*(translateX-width/2)+spawnPosX*width, max(playerY+height/2+40*scaling, -height/2+40*scaling), 5*((healthScale-1)/1.5+1)*diffStatScale, 1));
                }
            break;
            case 2: //Spawns enemy on left side of screen
              if (playerX - 80*scaling - width/2 > -1.5*width) {
                  NMEs.push(new NME(min(playerX-width/2-40*scaling, width/2-40*scaling), -1*(translateY-height/2)-spawnPosY*width, 5*((healthScale-1)/1.5+1)*diffStatScale, 1));
                }
              else { //Spawns enemy on right side of screen if left side is out of bounds
                  NMEs.push(new NME(max(playerX+width/2+40*scaling, -width/2+40*scaling), -1*(translateY-height/2)-spawnPosY*height, 5*((healthScale-1)/1.5+1)*diffStatScale, 1));
                }
            break;
            ...
            case 4: //Spawns enemy at the right side of the screen
              if (playerX + 80*scaling + width/2 < 1.5*width) {
                  ...
                  NMEs.push(new NME(min(playerX-width/2-40*scaling, width/2-40*scaling), -1*(translateY-height/2)-spawnPosY*height, 5*((healthScale-1)/1.5+1)*diffStatScale, 1))
                }
            break;
        }
        ...
        switch(ceil(random(4))) //Chooses random number 1 to 4 and spawns enemies on the side of the screen corresponding to that number
        {
          case 1: //Spawn enemy on top edge of screen
              if (playerY - 80*scaling - height/2 > -1.5*height) {
                  ...
                  NMEs.push(new NME(-1*(translateX-width/2)+spawnPosX*width, max(playerY+height/2+40*scaling, -height/2+40*scaling), 1*((healthScale-1)+1)*diffStatScale, 2));
                }
            break;
            case 2: //Spawns enemy on left side of screen
              if (playerX - 80*scaling - width/2 > -1.5*width) {
                  NMEs.push(new NME(min(playerX-width/2-40*scaling, width/2-40*scaling), -1*(translateY-height/2)-spawnPosY*width, 1*((healthScale-1)+1)*diffStatScale, 2));
                }
              else { //Spawns enemy on right side of screen if left side is out of bounds
                  NMEs.push(new NME(max(playerX+width/2+40*scaling, -width/2+40*scaling), -1*(translateY-height/2)-spawnPosY*height, 1*((healthScale-1)+1)*diffStatScale, 2));
                }
            break;
            case 3: //Spawns enemey at bottom of screen
              if (playerY + 80*scaling +height/2 < 1.5*height) {
                  ...
                  NMEs.push(new NME(-1*(translateX-width/2)+spawnPosX*width, min(playerY-height/2-40*scaling, height/2-40*scaling), 1*((healthScale-1)+1)*diffStatScale, 2));
                }
            break;
            case 4: //Spawns enemy at the right side of the screen
              if (playerX + 80*scaling + width/2 < 1.5*width) {
                ...
                NMEs.push(new NME(min(playerX-width/2-40*scaling, width/2-40*scaling), -1*(translateY-height/2)-spawnPosY*height, 1*((healthScale-1)+1)*diffStatScale, 2));
                }
            break;
        }
        ...
        switch(ceil(random(4))) //Chooses random number 1 to 4 and spawns enemies on the side of the screen corresponding to that number
        {
          case 1: //Spawn enemy on top edge of screen
              if (playerY - 120*scaling - height/2 > -1.5*height) {
                  NMEs.push(new NME(-1*(translateX-width/2)+spawnPosX*width, min(playerY-height/2-80*scaling, height/2-80*scaling), 12*((healthScale-1)*2+1)*diffStatScale, 3));
                }
              else { //Spawns enemy on bottom edge of screen if top edge is outside of border
                  NMEs.push(new NME(-1*(translateX-width/2)+spawnPosX*width, max(playerY+height/2+80*scaling, -height/2+80*scaling), 12*((healthScale-1)*2+1)*diffStatScale, 3));
                }
            break;
            case 2: //Spawns enemy on left side of screen
              if (playerX - 120*scaling - width/2 > -1.5*width) {
                  NMEs.push(new NME(min(playerX-width/2-80*scaling, width/2-80*scaling), -1*(translateY-height/2)-spawnPosY*width, 12*((healthScale-1)*2+1)*diffStatScale, 3));
                }
              else { //Spawns enemy on right side of screen if left side is out of bounds
                  NMEs.push(new NME(max(playerX+width/2+80*scaling, -width/2+80*scaling), -1*(translateY-height/2)-spawnPosY*height, 12*((healthScale-1)*2+1)*diffStatScale, 3));
                }
            break;
            case 3: //Spawns enemey at bottom of screen
              if (playerY + 120*scaling +height/2 < 1.5*height) {
            ...
            case 4: //Spawns enemy at the right side of the screen
              if (playerX + 120*scaling + width/2 < 1.5*width) {
                  ...
                  NMEs.push(new NME(min(playerX-width/2-80*scaling, width/2-80*scaling), -1*(translateY-height/2)-spawnPosY*height, 12*((healthScale-1)*2+1)*diffStatScale, 3));
                }
            break;
        }
